...
//   1) Call API — Method GET, URL or Template: #feedUrl#, Auth: none
//      Response field: rssXml, Response target: Forward to next flow step only
//      Uncheck "Append entry fields as GET query parameters" for RSS feeds
//   2) Run script (this file)
//
// rssXml is only the API response field name — do not add it to the config profile.
//
...
//   title, url (URL type), publishedAt (ISO UTC, sortable), summary (plain text), sourceName, feedGuid
//
// Set profile sort key to publishedAt, direction desc (newest first).
//
// HTML in feed descriptions is stripped to plain text; block tags and <br> become line breaks.
//
// Flow on config entry (timer or flow button with entry context):
//   Call API -> JS Processing -> Update current document (Param: lastImported)
//
...
function htmlToPlainText(html) {
  var s = String(html || "");
  // RegExp("...") for script/style patterns (not /.../ literals — safer when editing in HTML).
...
/** RSS pubDate (RFC 822) and Atom published/updated (ISO 8601) -> sortable UTC ISO string. */
